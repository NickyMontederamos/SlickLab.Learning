...
-- join code, everyone votes on candidate stg_topics from their own unlocked
-- set, the winning stg_topics become one shared full-length exam, everyone
...
-- No candidate-stg_topics column: the host's chosen candidates are recorded as
-- their own initial votes (see exhibition_create.php), so "the set of
-- stg_topics this session can be voted on" is simply
-- `SELECT DISTINCT topic_id FROM stg_exhibition_votes WHERE session_id = ?` --
...
-- recomputed from stg_exhibition_votes via csa_tally_exhibition_votes() --
...
-- (COUNT(DISTINCT user_id) in stg_exhibition_votes for the session), which
...
-- stg_exam_attempts.correct_count/duration_seconds for completed 'custom'
-- attempts sharing an exhibition_session_id, never stored twice.
CREATE TABLE IF NOT EXISTS stg_exhibition_sessions (
    ...
    FOREIGN KEY (host_user_id) REFERENCES stg_users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS stg_exhibition_votes (
    id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    session_id INT UNSIGNED NOT NULL,
    user_id INT UNSIGNED NOT NULL,
    topic_id INT UNSIGNED NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (session_id) REFERENCES stg_exhibition_sessions(id) ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES stg_users(id) ON DELETE CASCADE,
    FOREIGN KEY (topic_id) REFERENCES stg_topics(id) ON DELETE CASCADE,
