(async () => {
  ...
  // Lobby roster banner: same 6 cutouts, but greyed out until that teammate
  // actually joins this specific room (see pollLobby's 'joined' class toggle below).
  ...
  document.getElementById('revealBanner').addEventListener('click', (e) => {
    ...
    API.battleReact(roomId, btn.dataset.emoji).catch(() => { /* fire-and-forget */ });
  });

  function speakText(text) {
    if (!('speechSynthesis' in window)) return;
    const synth = window.speechSynthesis;
    // Chrome (and others) can silently drop an utterance queued in the same tick
    // right after cancel() — a well-known bug. Only cancel if something is actually
    // queued, and give it a beat to flush before speaking the new line.
  ...
  // Web Speech API is a separate subsystem from the Web Audio API — resuming an
  // AudioContext (BattleSound.init()) does nothing for speechSynthesis. Some
  // browsers (especially mobile) silently drop speak() calls unless the engine
  // was first touched directly inside a real user gesture, and later async calls
  // (from the poll loop) don't count as one. Call this from every click handler
  // that already calls BattleSound.init(), so it's primed for host and joiners alike.
    ...
    if (secLeft <= 0) {
      ...
      if (lastCountdownBeepValue !== secLeft) {
        ...
        if (overlay) {
          ...
          void overlay.offsetWidth; // restart the pulse animation for each new number
  ...
  async function pollIntroLobby() {
    let data;
    try {
      data = await API.battleLobby();
    } catch (e) {
      return; // non-critical, just skip this tick
    }
  ...
  document.getElementById('openRoomsList').addEventListener('click', async (e) => {
    ...
    primeSpeech();
    try {
      const res = await API.battleJoinRoom(parseInt(btn.dataset.joinRoom, 10));
      roomId = res.roomId;
      enterLobby();
    } catch (e2) {
  ...
  // --- Sound: init on first gesture, wire mute toggle ---
  ...
  document.getElementById('createBtn').addEventListener('click', async () => {
    ...
    try {
      const res = await API.battleCreate(itemCount, winningScore, ttsEnabled);
      roomId = res.roomId;
      enterLobby();
    } catch (e) {
  ...
  async function pollLobby() {
    let state;
    try {
      state = await API.battleRoom(roomId);
    } catch (e) {
      stopPolling();
      BattleMusic.stop();
    ...
    document.getElementById('lobbyParticipants').innerHTML = state.participants.map(p => `
      <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid var(--border);">
        <span style="display:flex; align-items:center; gap:8px;">${avatarHtml(p.username, 28)}${escapeHtml(p.username)}${p.isHost ? ' <span class="muted">(host)</span>' : ''}</span>
    ...
    readyBtn.onclick = async () => {
      BattleSound.init();
      primeSpeech();
      await API.battleReady(roomId, !state.meReady);
      pollLobby();
    };
  ...
  document.getElementById('leaveLobbyBtn').addEventListener('click', async () => {
    stopPolling();
    BattleMusic.stop();
    try { await API.battleLeave(roomId); } catch (e) { /* non-fatal */ }
    roomId = null;
    showScreen(introScreen);
  });
  ...
  async function confirmEndBattle() {
    ...
    if (!ok) return;
    try { await API.battleEnd(roomId); } catch (e) { notify(e.message); }
  }
  ...
  document.getElementById('quitBattleBtn').addEventListener('click', async () => {
    ...
    stopPolling();
    BattleMusic.stop();
    try { await API.battleQuit(roomId); } catch (e) { /* non-fatal, we're leaving anyway */ }
    roomId = null;
    showScreen(introScreen);
  });
  ...
  async function pollBattle() {
    let state;
    try {
      state = await API.battleState(roomId);
    } catch (e) {
      stopPolling();
      BattleMusic.stop();
    ...
    if (state.status === 'left') {
      // We already quit (likely from another tab/device) — just bail out quietly.
      stopPolling();
      BattleMusic.stop();
      roomId = null;
      showScreen(introScreen);
    ...
    // Scoreboard (podium cards)
    renderPodiumRow(state.participants);
    renderScoreRace(state.participants);

    // Reveal banner for the question that just ended
    ...
    // Incoming reactions from any player, float them up on everyone's screen
  ...
  function showPauseBanner(state) {
    ...
    if (state.isHost) {
      ...
      kickBtn.onclick = async () => {
        try { await API.battleKick(roomId, state.disconnectedUserId); } catch (e) { notify(e.message); }
      };
  ...
  function renderQuestion(state) {
    ...
    if (state.answersLocked) {
      updateLockUI(); // countdown/reading text is driven by the shared per-second updater
    } else {
    ...
    document.querySelectorAll('#battleOptions .option-row').forEach(row => {
      row.addEventListener('click', async () => {
        ...
        if (q.chooseN > 1) {
          ...
          renderQuestion(state);
          try {
            const res = await API.battleAnswer(roomId, myPendingSelection);
  ...
  const active = await API.battleActive();
  if (active.active) {
