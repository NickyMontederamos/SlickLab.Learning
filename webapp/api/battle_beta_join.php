...
$pdo = csa_db();
if ($roomIdIn) {
    $stmt = $pdo->prepare("SELECT id, status FROM battle_beta_rooms WHERE id = ?");
    $stmt->execute([$roomIdIn]);
} else {
    $stmt = $pdo->prepare("SELECT id, status FROM battle_beta_rooms WHERE code = ?");
    $stmt->execute([$code]);
}
$room = $stmt->fetch();
...
$stmt = $pdo->prepare('SELECT id FROM battle_beta_participants WHERE room_id = ? AND user_id = ?');
...
$countStmt = $pdo->prepare('SELECT COUNT(*) FROM battle_beta_participants WHERE room_id = ?');
$countStmt->execute([$roomId]);
if ((int)$countStmt->fetchColumn() >= $maxParticipants) {
    json_error('This room is full', 409);
}

$pdo->prepare('INSERT INTO battle_beta_participants (room_id, user_id, is_host, is_ready) VALUES (?, ?, 0, 0)')
    ->execute([$roomId, $uid]);
