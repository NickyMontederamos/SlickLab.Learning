...
function battle_fetch_room(PDO $pdo, int $roomId): ?array
{
...
function battle_participants_with_answered(PDO $pdo, int $roomId, int $index): array
{
    $stmt = $pdo->prepare(
        'SELECT p.user_id, u.username, p.score, p.current_streak,
                EXISTS(SELECT 1 FROM battle_answers a WHERE a.room_id = p.room_id AND a.user_id = p.user_id AND a.question_index = ?) AS answered
         FROM battle_participants p JOIN users u ON u.id = p.user_id
...
function battle_question_by_id(PDO $pdo, int $qid, bool $withCorrect): array
{
...
function battle_lock_seconds_for_question(PDO $pdo, int $qid, bool $ttsEnabled): float
{
    if (!$ttsEnabled) {
        return 0.0;
    }
...
function battle_final_results(PDO $pdo, int $roomId): array
{
    $stmt = $pdo->prepare(
        'SELECT p.user_id, u.username, p.score, p.best_streak FROM battle_participants p
         JOIN users u ON u.id = p.user_id WHERE p.room_id = ? AND p.status = "active"
    ...
    $mvpStmt = $pdo->prepare(
        'SELECT user_id FROM battle_answers
...
$room = battle_fetch_room($pdo, $roomId);
if (!$room) {
...
if ($room['status'] === 'finished') {
    json_out(['status' => 'finished', 'results' => battle_final_results($pdo, $roomId)]);
}

// Has the calling user themselves been kicked or already left (e.g. a stale second
// tab/device still polling after they quit elsewhere)? Tell their own client so it can bail out.
$myStmt = $pdo->prepare('SELECT status FROM battle_participants WHERE room_id = ? AND user_id = ?');
...
$pdo->prepare("UPDATE battle_participants SET last_seen_at = NOW() WHERE room_id = ? AND user_id = ? AND status = 'active'")
    ->execute([$roomId, $uid]);

// --- Paused: check whether the flagged player has come back, otherwise stay paused. ---
if ($room['status'] === 'paused') {
    $discUserId = $room['disconnected_user_id'] !== null ? (int)$room['disconnected_user_id'] : null;
    $backOnline = false;
    if ($discUserId !== null) {
        $stmt = $pdo->prepare('SELECT last_seen_at FROM battle_participants WHERE room_id = ? AND user_id = ? AND status = "active"');
        ...
        $backOnline = true; // disconnected player was already removed/kicked elsewhere
    }
    ...
    if ($backOnline) {
        ...
        $room = battle_fetch_room($pdo, $roomId);
        // falls through to the in_progress handling below
    } else {
        ...
        json_out([
            ...
            'participants' => battle_participants_with_answered($pdo, $roomId, (int)$room['current_index']),
        ]);
    }
}

// status === 'in_progress': check whether it's time to advance, or whether someone just dropped.
...
$lockSeconds = $lockQid !== null ? battle_lock_seconds_for_question($pdo, $lockQid, $ttsEnabled) : 0.0;
$totalWindow = $lockSeconds + $questionSeconds;

$stmt = $pdo->prepare("SELECT user_id, last_seen_at FROM battle_participants WHERE room_id = ? AND status = 'active'");
$stmt->execute([$roomId]);
$activeParticipants = $stmt->fetchAll();
$totalParticipants = count($activeParticipants);

// Detect a stale (likely disconnected) participant other than the caller.
foreach ($activeParticipants as $p) {
    $pid = (int)$p['user_id'];
    if ($pid === $uid) {
        continue; // we just proved we're alive above
    }
    if ($p['last_seen_at'] !== null && (time() - strtotime($p['last_seen_at'])) >= $DISCONNECT_THRESHOLD) {
        $pdo->prepare(
            "UPDATE battle_rooms SET status = 'paused', paused_at = NOW(), disconnected_user_id = ?
        ...
        if ($room['status'] === 'paused') {
            ...
            json_out([
                ...
                'isHost' => (int)$room['host_user_id'] === $uid,
                'participants' => battle_participants_with_answered($pdo, $roomId, $currentIndex),
            ]);
        }
        break;
    }
...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM battle_answers WHERE room_id = ? AND question_index = ?');
...
if ($needAdvance) {
    ...
    $stmt = $pdo->prepare("SELECT COALESCE(MAX(score),0) FROM battle_participants WHERE room_id = ? AND status = 'active'");
    $stmt->execute([$roomId]);
    ...
    if ($winnerReached || $nextIndex >= $itemCount) {
        $pdo->prepare("UPDATE battle_rooms SET status = 'finished', finished_at = NOW() WHERE id = ? AND status = 'in_progress' AND current_index = ?")
            ->execute([$roomId, $currentIndex]);
    } else {
        $pdo->prepare("UPDATE battle_rooms SET current_index = ?, question_started_at = NOW() WHERE id = ? AND status = 'in_progress' AND current_index = ?")
            ->execute([$nextIndex, $roomId, $currentIndex]);
    }

    // Re-fetch authoritative state after the (possibly concurrent) advance attempt.
    $room = battle_fetch_room($pdo, $roomId);
    $currentIndex = (int)$room['current_index'];
    $startedAt = strtotime($room['question_started_at']);
    $elapsed = time() - $startedAt;
...
if ($currentIndex > 0 && isset($questionIds[$currentIndex - 1])) {
    ...
    $prevQuestion = battle_question_by_id($pdo, $prevQid, true);
    $stmt = $pdo->prepare(
        'SELECT a.user_id, u.username, a.selected_letters, a.is_correct, a.seconds_taken FROM battle_answers a
...
$myAnswerStmt = $pdo->prepare('SELECT selected_letters FROM battle_answers WHERE room_id = ? AND user_id = ? AND question_index = ?');
...
$reactStmt = $pdo->prepare(
    'SELECT r.id, r.user_id, u.username, r.emoji FROM battle_reactions r
...
json_out([
    ...
    'question' => $currentQid !== null ? battle_question_by_id($pdo, $currentQid, false) : null,
