...
$pdo = csa_db();
$stmt = $pdo->prepare("SELECT status, host_user_id FROM battle_rooms WHERE id = ?");
$stmt->execute([$roomId]);
$room = $stmt->fetch();
if (!$room) {
...
$pdo->prepare('DELETE FROM battle_participants WHERE room_id = ? AND user_id = ?')->execute([$roomId, $uid]);
...
if ((int)$room['host_user_id'] === $uid) {
    $pdo->prepare('DELETE FROM battle_rooms WHERE id = ?')->execute([$roomId]);
}
